...
module.exports = async(messages, sock, store) => {
	...
	if (messages.key) {
		m.key = messages.key;
		m.cht = m.key.remoteJid.startsWith('status') ? jidNormalizedUser(m.key?.participant || messages.participant) : jidNormalizedUser(m.key.remoteJid);
		m.fromMe = m.key.fromMe;
		m.id = m.key.id;
		m.isBot = m?.id.startsWith('BAE5') || (m?.id.indexOf("-") > 1) || m?.id.startsWith('3EB0') || (m?.id.indexOf("LTS-") > 1);
		m.isGroup = m.cht.endsWith('@g.us');
		m.participant = jidNormalizedUser(messages?.participant || m.key.participant) || false;
		m.sender = jidNormalizedUser(m.fromMe ? sock.user.id : m.isGroup ? m.participant : m.cht);
	}
	if (m.isGroup) {
		if (!(m.cht in store.groupMetadata)) store.groupMetadata[m.cht] = await sock.groupMetadata(m.cht);
		...
		m.isBotAdmin = m.isGroup && !!m.groupAdmins.find(member => member.id === jidNormalizedUser(sock.user.id));
	}
	m.pushName = messages.pushName;
	m.isOwner =  [sock.decodeJid(sock.user.id),...(config.owner.map(a => a + "@s.whatsapp.net"))].includes(m.sender)
	if (m.message) {
		...
		if (m.msg?.contextInfo?.quotedMessage) {
			...
			if (m.quoted.message) {
				...
				m.quoted.cht = /g\.us|status/.test(m.msg?.contextInfo?.remoteJid) ? m.quoted.key.participant : m.quoted.key.remoteJid;
				...
				m.quoted.isBot = m?.id.startsWith('-') || m.quoted.id.startsWith('BAE5') || m.quoted.id.startsWith('HSK') || (m.id.indexOf("LTS-") > 1);
				m.quoted.isGroup = m.quoted.cht.endsWith('@g.us');
				m.quoted.participant = jidNormalizedUser(m.msg?.contextInfo?.participant) || false;
				m.quoted.sender = jidNormalizedUser(m.msg?.contextInfo?.participant || m.quoted.cht);
    ...
    let msg = {
      ...
      type: "append",
       ...
       return sock.ev.emit("messages.upsert", msg);
				...
				m.quoted.isOwner =   [sock.decodeJid(sock.user.id), ...(config.owner.map(a => a + "@s.whatsapp.net"))].includes(m.sender)
//==========[ Function bot ]=============//
if (m.quoted.isMedia) {
...
if (m.isMedia) {
 ...
 m.reply = async (text, options = {}) => {
     ...
     await sock.sendMessage(m.cht, { text, mentions: [...sock.parseMention(text)], ...options }, { quoted: m, ephemeralExpiration: m.expiration, ...options });
		} else if (typeof text === 'object' && typeof text !== 'string') {
	 await sock.sendMessage(m.cht, { ...text, ...options }, { quoted: m, ephemeralExpiration: m.expiration, ...options });
		}
	};
  m.react = async(emoji) => {
