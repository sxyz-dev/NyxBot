...
module.exports = {
  ...
  gofile: async (content) => {
    try {
      ...
      const getServer = await axios.get("https://api.gofile.io/getServer").then(a => a.data);
      const response = await axios.post(`https://${getServer.data.server}.gofile.io/uploadFile`, formData, {
          headers: {
            "User-Agent": fakeUserAgent(),
          },
        });
  ...
  media: async (buffer) => {
    try {
      ...
      const formData = await createFormData(buffer, "files[]", ext),
        response = await axios.post("https://media-upload.net/php/ajax_upload_file.php", formData, {
            headers: {
              "User-Agent": fakeUserAgent(),
            },
          });
 ...
 caliph: async (buffer) => {
    try {
      ...
      let response = await axios.request("https://filezone-api.caliph.dev/upload", {
        method: "POST",
        data: formData.getBuffer(),
        headers: {
          ...formData.getHeaders(),
 ...
 doods: async (buffer) => {
    try {
      ...
      const server = await axios.get("https://doodapi.com/api/upload/server?key=13527p8pcv54of4yjeryk").then(a => a.data);
